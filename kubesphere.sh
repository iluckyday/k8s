...
sleep 5
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p 22222 -l root 127.0.0.1 kubectl get pods -A
...
while [ true ]; do
  pid=`pgrep kubesphere-building2 || true`
  if [ -z $pid ]; then
    break
  else
    sleep 2
...
qemu-img convert -c -f raw -O qcow2 /tmp/debian2.raw /tmp/kubesphere-${KVERSION}.img.k8s
